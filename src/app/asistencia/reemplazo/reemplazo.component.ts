...
export class ReemplazoComponent {
  ...
  initializeForm() {
    this.asistenciaForm = this.fb.group({
      ...
      insertApplicationName:
        this.aula == null ? "Intranet Docentes" : this.aula,
      horaEntrada: [],
      horaSalida: [],
      modalidad: "",
    });
  ...
  obtenerHorario() {
    ...
    this.horarioService.obtenerReemplazo(this.profesor.idProfesor).subscribe({
      next: (datos) => {
        if (datos.length > 0) {
          ...
          this.asistenciaForm.patchValue({
            ...
            descripcion:
              moment(datos[0].fechaInicio).format("hh:mm") +
              " - " +
              moment(datos[0].fechaFinal).format("hh:mm"),
  ...
  entrada() {
    ...
    if (objentrada.modalidad == "Presencial") {
      this.snakBar.open(
        "Es un horario presencial debes hacerlo desde la aplicación de escritorio",
        "OK",
        {
      ...
      this.asistenciaDocenteService
        .EntradaReemplazo(objentrada)
        .subscribe(() => {
          this.mostrarrefresh = false;
          this.obtenerHorario();
